	...
	<div class="container-fluid">
		...
		<div class="row">
			<div class="col-lg-5 col-md-12">
				...
				<h3> Add Order </h3>
				<p> <small> <i class="fas fa-chart-pie"></i> </small> <small> Command Center </small> > <small> Command Center </small> > <small> Add Order </small> </p>
				</div>
			</div>
			...
			<div class="col-lg-9">
				...
				<div class="add-new-order-tabss-nav">
					...
					<div class="card-body" id="msform">
						...
						<!-- <div class="tab-content custmer_order_pills"> -->
								...
								<div class="row">
									<div class="col-lg-4 col-md-12">
										<div class="d-flex align-items-center justify-content-between bg-white p-2 border border-info rounded">
											...
											@endforeach
											@endif
											<a class="commandcenteraddorder" data-customer_order_id="{{$order_id}}" href="#" data-bs-toggle="modal" data-bs-target="#commandcenteraddorderModal"><p class="text-danger mb-0">Change</p></a>
										</div>
										@if(isset($customercars))
										@foreach($customercars->data as $car)
									<div class="vichle-detailss">
										<input type="checkbox" id="select-cars-commandcenter" class="select-cars" data-car_id="@if(!empty($car->_id)){{$car->_id}}@endif" data-ot_type="@if(!empty($ot_type)){{$ot_type}}@endif" data-ot_id="@if(!empty($ot_id)){{$ot_id}}@endif" data-car_image="@if(!empty($car->make_by->image)){{$car->make_by->image}}@endif" data-car_color="@if(!empty($car->color->name)){{$car->color->name}}@endif" data-car_year="@if(!empty($car->car_year)){{$car->car_year}}@endif" data-car_model="@if(!empty($car->car_model)){{$car->car_model}}@endif" data-car_name="@if(!empty($car->make_by->name)){{$car->make_by->name}}@endif" data-car_type="@if(!empty($car->car_type)){{$car->car_type}}@endif" data-subscription_id="@if(!empty($car->subscription_id)){{$car->subscription_id}}@endif" data-action_url="{{ route('selectvehicle_add_order') }}" >
									...
									<div class="col-lg-8 col-md-12">
											...
											<div class="iframe-car-vichless">
												...
												<div id="map_wrapper">
													...
													center: {lat: @if(!empty($lat)) {{$lat}} @endif , lng: @if(!empty($long)) {{$long}} @endif },
													zoom: 9,
												...
												<div class="accordion__item">
													...
													<div id="default_collapseOne" class="accordion__body collapse show" data-parent="#accordion-one" style="">
														<div class="accordion__body--text">
															...
															<div class="time-formate" id="available-time">
						...
						<fieldset>
							...
							<a href="{{route('commandcenterview')}}" class="danger-custom-btn wizard_cancle_btn"><i class="fas fa-times"></i>  Cancel  </a>
							<button type="submit" onclick="document.getElementById('createorderButton').click()" class="action-button gree-custom-btn wizard_next_btn">Submit <i class="fas fa-chevron-right"></i></button>
						</fieldset>			
						<!-- </div> -->
					</div>
			</div>
...
<!--==== ====-->
<form method="POST" action="{{route('commandcentercreateorder')}}" onsubmit="return ValidationForm()" >
 @csrf
									...
									jQuery(document).ready(function() {
										jQuery('.calendar').datepicker({
											...
											onSelect: function () {
												...
												var action_url = "{{route("timeavailable_add_order")}}";
