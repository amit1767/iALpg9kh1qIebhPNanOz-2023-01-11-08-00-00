	<ul class="nav nav-tabs" role="tablist" id="started-task-data">
			@if(!empty($started_orderlist->data))
				@php $i=0; @endphp
				@foreach($started_orderlist->data as $staff_order)
				@php $i++; @endphp
			  ...
			  <li role="presentation" >
			   <a href="#Sections2-{{$i}}" aria-controls="#Sections2-{{$i}}" @if($i==1)class="active" @endif role="tab" data-toggle="tab" data-value="{{$i}}" onclick="tracking_details('@if(!empty($staff_order->bucket_id)){{$staff_order->bucket_id}}@endif',$(this).data('value'))" >
					...
					<div class="order-main-box-nav">
					  <div class="customer-icon-box-up">
					   <div class="customer-sn">
						...
						@if(!empty($staff_order->time_enroute) && !empty($staff_order->duration->value)) @php $tot_tm=strtotime($staff_order->time_enroute)+$staff_order->duration->value;  @endphp @endif @if(!empty($staff_order->time_started) && !empty($staff_order->time_enroute) && !empty($staff_order->duration->value)) @php $late_b = (strtotime($staff_order->time_enroute)+$staff_order->duration->value) - strtotime($staff_order->time_started); $late_by = $late_b/60; $lateby = (int)$late_by; @endphp @endif
						<div class="customer-detail-02"> 
						  	<!-- <img src="{{ asset('public/assets/image/iccc.png')}}" alt=""> -->
						 	<p class="customer-detail-002">
								<img src="@if(!empty($staff_order->cleaner->avatar->default_path)){{$staff_order->cleaner->avatar->default_path}}@endif" alt="" class="customer_box_icon_img"> @if(!empty($staff_order->cleaner->cleaner_first_name)){{$staff_order->cleaner->cleaner_first_name}}@endif @if(!empty($staff_order->cleaner->cleaner_last_name)){{$staff_order->cleaner->cleaner_last_name}}@endif </span>
							</p>
						  	<p class="" style="color: #0FCA93;"><i class="far fa-clock"></i> Started At: @if(!empty($staff_order->time_started)) {{ date('d M Y h:i A',strtotime($staff_order->time_started))}} @endif</p>
					  ...
					  <div class="zones-reach-boxx">
						<div class="zones-reach-text">
							...
							<p class="pan_t_add"> <strong>@if(!empty($staff_order->address->label)){{$staff_order->address->label}}@endif:</strong> 
							<span>@if(!empty($staff_order->address->description)) {{$staff_order->address->description}} @endif</p>
                ...
                <!-- Tab panes -->
  	   <div class="tab-content tabs commond_sidebar_tabcontent" id="started-task-data1">
			@if(!empty($started_orderlist->data))
				@php $i=0; @endphp
				@foreach($started_orderlist->data as $staff_order)
				@php $i++; @endphp
		  <div role="tabpanel" class="commond_side_bar_fixed tab-pane @if($i==1) active @endif" id="Sections2-{{$i}}">
				...
				<div class="customer_online_detail_box">
					<div class="d-flex align-items-center justify-content-between">
						<div class="customer_online_detail">						
							<a href="#" class="task_profile_img_4"><img src="@if(!empty($staff_order->cleaner->avatar->default_path)){{$staff_order->cleaner->avatar->default_path}}@endif" alt="" class="iconn-image">
							<span class="task_profile_online_dot_4"></span></a>
							<div>
								...
								<p style="color: #fc0800;">Started</p>
							</div>												
						</div>
			    ...
			    <div class="body-small-details assign-ninja-boxx">
			   	...
			   	<div class="tracking-detail-box">
				  	<div class="traking_details">
					    ...
					    <div class="accordion" id="started_tracking_accordion-{{$i}}">
		...
		<form action="" method="post" name="" id="" >
			...
			<button type="button" class="endtask-custom-btn" > End Task </button>
		</form>
		</div> 
		</div>
		@endforeach
